...
type UseCase =
  | "Campaigns" | "A&R Discovery" | "Release Planning" | "Creator Marketing"
  | "Reporting/Analytics" | "Rights/IP" | "Other";

type SignupPayload = {
  role: Role; fullName: string; email: string;
  org?: string; primaryArtist?: string; location?: string;
  website?: string; instagram?: string; spotify?: string; soundcloud?: string;
  useCases: UseCase[]; notes?: string; marketingConsent: boolean;
  utmSource?: string | null; utmMedium?: string | null; utmCampaign?: string | null; referrer?: string | null;
  enriched?: { typeGuess?: string; managerGuess?: string; notableWorks?: string[]; confidence?: number; provenance?: string[]; };
};

const USE_CASES: UseCase[] = ["Campaigns","A&R Discovery","Release Planning","Creator Marketing","Reporting/Analytics","Rights/IP","Other"];
...
export default function SignupPage() {
  ...
  const [form, setForm] = useState<SignupPayload>({
    role: "Artist", fullName: "", email: "", org: "", primaryArtist: "", location: "",
    website: "", instagram: "", spotify: "", soundcloud: "", useCases: [], notes: "",
    marketingConsent: true, utmSource: null, utmMedium: null, utmCampaign: null,
    referrer: typeof window !== "undefined" ? document.referrer || null : null,
  });

  useEffect(() => {
    setForm((f) => ({ ...f, utmSource: sp.get("utm_source"), utmMedium: sp.get("utm_medium"), utmCampaign: sp.get("utm_campaign") }));
  }, [sp]);

  const canSubmit = useMemo(() => !!form.fullName && !!form.email && form.marketingConsent && !submitting, [form, submitting]);

  function toggleUseCase(uc: UseCase) {
    setForm((f) => ({ ...f, useCases: f.useCases.includes(uc) ? f.useCases.filter(x => x !== uc) : [...f.useCases, uc] }));
  }
  ...
  function aiAssist() {
    ...
    const key = subject.toLowerCase(); const hit = seeds[key];
    const enriched = {
      typeGuess, managerGuess: hit?.manager || (typeGuess === "Producer" ? "TBD (producer mgmt?)":"TBD"),
      notableWorks: hit?.works || (typeGuess === "Producer" ? ["Recent placements?"] : ["Top tracks?"]),
      confidence: hit ? 0.8 : 0.45, provenance: ["heuristic:name+org", hit ? `seed:${key}` : "seed:none","future:web+internal graph"],
    };
    setForm((f) => ({ ...f, role: enriched.typeGuess as Role, notes: (f.notes?f.notes+"\n":"")+`Enriched: manager≈${enriched.managerGuess}; works≈${enriched.notableWorks?.join(", ")}`, enriched }));
  }

  async function onSubmit(e: React.FormEvent) {
    e.preventDefault(); if (!canSubmit) return; setSubmitting(true); setError(null);
    try {
      const res = await fetch("/api/signup", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(form) });
      if (!res.ok) throw new Error(await res.text());
      const data = (await res.json()) as { ok: true; id: string; storage: "kv"|"log" };
    ...
    return (
      <div className="p-6 max-w-3xl mx-auto">
        <Card><CardContent className="p-6 space-y-3">
          <div className="text-xl font-semibold">Thanks — you’re on the list.</div>
          <div className="text-sm text-muted-foreground">We’ll use this info to personalize your Co-Pilot and reach out with the right next steps.</div>
          <div className="text-xs text-muted-foreground">Ref <Badge variant="outline">{submitted.id}</Badge></div>
          <div className="text-sm">Back to <a className="underline" href="/">home</a></div>
        </CardContent></Card>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <div className="mb-4">
        ...
        <div className="text-sm text-muted-foreground">Tell us who you are. We’ll personalize your workspace and keep you posted.</div>
      </div>
      <form onSubmit={onSubmit} className="grid gap-4">
        <Card><CardContent className="p-4 grid gap-3">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div className="grid gap-2">
              ...
              <Select value={form.role} onValueChange={(v)=>setForm((f)=>({...f, role: v as Role}))}>
                ...
                <SelectContent>{(["Artist","Producer","Manager","Label","Other"] as Role[]).map((r)=><SelectItem key={r} value={r}>{r}</SelectItem>)}</SelectContent>
              </Select>
            </div>
            <div className="grid gap-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <div className="grid gap-2"><div className="text-sm font-medium">Website</div><Input value={form.website} onChange={(e)=>setForm({...form, website:e.target.value})} placeholder="https://…" /></div>
            <div className="grid gap-2"><div className="text-sm font-medium">Instagram</div><Input value={form.instagram} onChange={(e)=>setForm({...form, instagram:e.target.value})} placeholder="@handle or link" /></div>
            <div className="grid gap-2"><div className="text-sm font-medium">Spotify</div><Input value={form.spotify} onChange={(e)=>setForm({...form, spotify:e.target.value})} placeholder="Artist/Team profile link" /></div>
          </div>
          ...
          <div className="flex gap-2">
            <Button type="button" variant="secondary" onClick={aiAssist} disabled={!form.fullName && !form.primaryArtist}>AI Assist — Prefill</Button>
            <Button type="submit" disabled={!canSubmit}>{submitting ? "Submitting…" : "Join Waitlist"}</Button>
