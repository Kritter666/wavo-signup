...
export default function SignupPageClient() {
  ...
  // Fill from query params (UTM + email)
  const qEmail = sp.get("email") || null;
  useEffect(() => {
    setForm((f) => ({
      ...f,
      email: qEmail ?? f.email,
  ...
  const emailCameFromWelcome = !!qEmail;
      ...
      <form onSubmit={onSubmit} className="grid gap-4">
        <Card>
          <CardContent className="p-4 grid gap-3">
            ...
            {/* Email section: hide input when it came from the first step */}
            {!emailCameFromWelcome ? (
              <div className="grid gap-2">
              ...
              <div className="rounded-lg border p-3 text-sm flex items-center justify-between">
                <span>
                  Using <span className="font-mono">{form.email}</span>
                </span>
                <button
                  type="button"
                  className="underline"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div className="grid gap-2">
                ...
                <Input
                  ...
                  onChange={(e) => setForm({ ...form, primaryArtist: e.target.value })}
